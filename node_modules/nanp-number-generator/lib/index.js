"use strict";
var __defProp = Object.defineProperty;
var __getOwnPropDesc = Object.getOwnPropertyDescriptor;
var __getOwnPropNames = Object.getOwnPropertyNames;
var __hasOwnProp = Object.prototype.hasOwnProperty;
var __export = (target, all) => {
  for (var name in all)
    __defProp(target, name, { get: all[name], enumerable: true });
};
var __copyProps = (to, from, except, desc) => {
  if (from && typeof from === "object" || typeof from === "function") {
    for (let key of __getOwnPropNames(from))
      if (!__hasOwnProp.call(to, key) && key !== except)
        __defProp(to, key, { get: () => from[key], enumerable: !(desc = __getOwnPropDesc(from, key)) || desc.enumerable });
  }
  return to;
};
var __toCommonJS = (mod) => __copyProps(__defProp({}, "__esModule", { value: true }), mod);

// src/index.ts
var src_exports = {};
__export(src_exports, {
  generateNANPNumber: () => generateNANPNumber
});
module.exports = __toCommonJS(src_exports);

// src/consts.ts
var AREA_CODES = [
  201,
  202,
  203,
  205,
  206,
  207,
  208,
  209,
  210,
  212,
  213,
  214,
  215,
  216,
  217,
  218,
  219,
  220,
  223,
  224,
  225,
  228,
  229,
  231,
  234,
  239,
  240,
  248,
  251,
  252,
  253,
  254,
  256,
  260,
  262,
  267,
  269,
  270,
  272,
  276,
  279,
  281,
  301,
  302,
  303,
  304,
  305,
  307,
  308,
  309,
  310,
  312,
  313,
  314,
  315,
  316,
  317,
  318,
  319,
  320,
  321,
  323,
  325,
  326,
  330,
  331,
  332,
  334,
  336,
  337,
  339,
  340,
  341,
  346,
  347,
  350,
  351,
  352,
  360,
  361,
  363,
  364,
  380,
  385,
  386,
  401,
  402,
  404,
  405,
  406,
  407,
  408,
  409,
  410,
  412,
  413,
  414,
  415,
  417,
  419,
  423,
  424,
  425,
  430,
  432,
  434,
  435,
  440,
  442,
  443,
  445,
  447,
  448,
  458,
  463,
  464,
  469,
  470,
  472,
  475,
  478,
  479,
  480,
  484,
  501,
  502,
  503,
  504,
  505,
  507,
  508,
  509,
  510,
  512,
  513,
  515,
  516,
  517,
  518,
  520,
  530,
  531,
  534,
  539,
  540,
  541,
  551,
  557,
  559,
  561,
  562,
  563,
  564,
  567,
  570,
  571,
  572,
  573,
  574,
  575,
  580,
  582,
  585,
  586,
  601,
  602,
  603,
  605,
  606,
  607,
  608,
  609,
  610,
  612,
  614,
  615,
  616,
  617,
  618,
  619,
  620,
  623,
  626,
  628,
  629,
  630,
  631,
  636,
  640,
  641,
  646,
  650,
  651,
  656,
  657,
  659,
  660,
  661,
  662,
  667,
  669,
  670,
  671,
  678,
  680,
  681,
  682,
  684,
  689,
  701,
  702,
  703,
  704,
  706,
  707,
  708,
  712,
  713,
  714,
  715,
  716,
  717,
  718,
  719,
  720,
  724,
  725,
  726,
  727,
  731,
  732,
  734,
  737,
  740,
  743,
  747,
  754,
  757,
  760,
  762,
  763,
  765,
  769,
  770,
  771,
  772,
  773,
  774,
  775,
  779,
  781,
  785,
  786,
  787,
  801,
  802,
  803,
  804,
  805,
  806,
  808,
  810,
  812,
  813,
  814,
  815,
  816,
  817,
  818,
  820,
  826,
  828,
  830,
  831,
  832,
  835,
  838,
  839,
  840,
  843,
  845,
  847,
  848,
  850,
  854,
  856,
  857,
  858,
  859,
  860,
  862,
  863,
  864,
  865,
  870,
  872,
  878,
  901,
  903,
  904,
  906,
  907,
  908,
  909,
  910,
  912,
  913,
  914,
  915,
  916,
  917,
  918,
  919,
  920,
  925,
  928,
  929,
  930,
  931,
  934,
  936,
  937,
  938,
  939,
  940,
  941,
  943,
  945,
  947,
  948,
  949,
  951,
  952,
  954,
  956,
  959,
  970,
  971,
  972,
  973,
  978,
  979,
  980,
  983,
  984,
  985,
  986,
  989
];

// src/validate/area-code.ts
var validateAreaCode = (code) => {
  if (code.length !== 3) {
    throw new Error(`Area code must be 3 digits`);
  }
  if (!AREA_CODES.find((areaCode) => areaCode.toString() === code)) {
    throw new Error(`Area code must be a valid NANP area code`);
  }
  return true;
};

// src/validate/central-office-code.ts
var validateCentralOfficeCode = (code) => {
  if (code.length !== 3) {
    throw new Error(`Central office code must be 3 digits`);
  }
  const digits = code.split(``);
  if (digits[0] === `0` || digits[0] === `1`) {
    throw new Error(`Central office code must not start with 0 or 1`);
  }
  if (digits[1] === `1` && digits[2] === `1`) {
    throw new Error(`Central office code must not end with 11`);
  }
  return true;
};

// src/validate/line-number.ts
var validateLineNumber = (code) => {
  if (code.length !== 4) {
    throw new Error(`Line number must be 4 digits`);
  }
  if (code.startsWith(`01`)) {
    throw new Error(`Line number must not end in range 0100-0199`);
  }
  return true;
};

// src/generate/area-code.ts
var generateAreaCode = (override) => {
  if (override) {
    if (validateAreaCode(override)) {
      return override;
    }
    throw new Error(`Invalid area code`);
  }
  return AREA_CODES[Math.floor(Math.random() * AREA_CODES.length)].toString();
};

// src/generate/central-office-code.ts
var generateCentralOfficeCode = (override) => {
  if (override) {
    if (validateCentralOfficeCode(override)) {
      return override;
    }
    throw new Error(`Invalid central office code`);
  }
  const first = Math.floor(Math.random() * 8) + 2;
  const second = Math.floor(Math.random() * 9);
  const third = Math.floor(Math.random() * 9);
  if (second === 1 && third === 1) {
    return generateCentralOfficeCode();
  }
  if (first === 5 && second === 5 && third === 5) {
    return generateCentralOfficeCode();
  }
  return `${first}${second}${third}`;
};

// src/generate/line-number.ts
var generateLineNumber = (override, fictionalize) => {
  if (override) {
    if (validateLineNumber(override)) {
      return override;
    }
    throw new Error(`Invalid line number`);
  }
  if (typeof fictionalize !== `undefined`) {
    const third2 = Math.floor(Math.random() * 10);
    const fourth2 = Math.floor(Math.random() * 10);
    return `01${third2}${fourth2}`;
  }
  const first = Math.floor(Math.random() * 10);
  const second = Math.floor(Math.random() * 10);
  const third = Math.floor(Math.random() * 10);
  const fourth = Math.floor(Math.random() * 10);
  if (first === 0 && second === 1) {
    return generateLineNumber();
  }
  return `${first}${second}${third}${fourth}`;
};

// src/generate/nanp-number.ts
var generateNANPNumber = (options) => {
  if (typeof options === `boolean`) {
    const areaCode = generateAreaCode();
    const centralOfficeCode = `555`;
    const lineNumber = generateLineNumber(void 0, true);
    return `${areaCode}${centralOfficeCode}${lineNumber}`;
  } else {
    let { areaCode, centralOfficeCode, lineNumber } = options;
    if (!areaCode || !validateAreaCode(areaCode)) {
      areaCode = generateAreaCode();
    }
    if (!centralOfficeCode || !validateCentralOfficeCode(centralOfficeCode)) {
      centralOfficeCode = generateCentralOfficeCode();
    }
    if (!lineNumber || !validateLineNumber(lineNumber)) {
      lineNumber = generateLineNumber();
    }
    return `${areaCode}${centralOfficeCode}${lineNumber}`;
  }
};
// Annotate the CommonJS export names for ESM import in node:
0 && (module.exports = {
  generateNANPNumber
});
//# sourceMappingURL=index.js.map